<svelte:head>
	<title>Games</title>
</svelte:head>

<div class="w-full base-font-color grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
	
	<a href="https://store.steampowered.com/app/3395730/Banshee_Bash/" target="_blank"
		class="card preset-filled card-hover overflow-hidden border-surface-500/30 border">
		
		<img src="/images/capsule_header.jpg" class="w-full" alt="Banshee Bash Header">
		<section class="p-4">
			<div>Banshee Bash is a local-multiplayer party game. </div>
			<div>Fight with up to 3 friends (or add bots) to battle as a team, or in an epic free-for-all. </div>
			<div>Collect mana and learn spells to cast your way to victory! </div>
			<div>Games are quick, and will allow you to level up for unlockable cosmetics to add some flair!</div>
		</section>
		<hr>
		<section class="p-4">
			<div>Coming Q2 2025</div>
		</section>
	</a>
</div>